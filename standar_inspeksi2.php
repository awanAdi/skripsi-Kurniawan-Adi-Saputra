...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_POST['edit_id'])) {
        ...
        $id_kriteria = (int)$_POST['edit_kriteria'];
...
<head>
    ...
    <style>
        ...
        /* ----- Pembesaran font tabel & spasi agar lebih terbaca ----- */
        #dataTable {
            font-size: 1rem;
            /* setara text-base; lebih besar dari text-sm */
            line-height: 1.45;
        }

        /* Input / textarea di dalam tabel agar nyaman dibaca */
        #dataTable input,
        #dataTable textarea,
        #dataTable select {
            font-size: 0.95rem;
        ...
        /* Buat textarea bisa di-resize vertikal dan nyaman */
        #dataTable textarea {
            resize: vertical;
            min-height: 40px;
            /* minimal 2 baris */
        ...
        /* Untuk cell tampilan non-editable, pastikan teks membungkus */
        #dataTable td {
            white-space: normal;
            word-break: break-word;
        }

        /* Pada tampilan kecil (mobile), beri sedikit spacing lebih */
                    ...
                    <tbody class="text-gray-700 relative">
                        <?php while ($s = $standar_result->fetch_assoc()): ?>
                            <?php if ($mode === 'detail'): ?>
                                ...
                                <?php if ($mode === 'detail'): ?>
                                    <td class="px-5 py-6">
                                        <input type="hidden" name="id_standar[]" value="<?= (int)$s['id_standar'] ?>">
                                    ...
                                    <td class="px-4 py-3">
                                        <select name="tipe_input[]" class="border p-1 rounded" disabled data-original="<?= htmlspecialchars($s['tipe_input']) ?>">
                                    ...
                                    <td class="px-4 py-3">
                                        <input type="text" name="nilai_batas[]" value="<?= htmlspecialchars($s['nilai_batas']) ?>" class="w-full border p-1 rounded" disabled data-original="<?= htmlspecialchars($s['nilai_batas']) ?>">
                                    </td>

                                    <td class="px-4 py-3">
                                        <input type="text" name="opsi_pilihan[]" value="<?= htmlspecialchars($s['opsi_pilihan']) ?>" class="w-full border p-1 rounded" disabled data-original="<?= htmlspecialchars($s['opsi_pilihan']) ?>">
                                    </td>

                                    <td class="px-4 py-3">
                                        <input type="text" name="keterangan[]" value="<?= htmlspecialchars($s['keterangan']) ?>" class="w-full border p-1 rounded" disabled data-original="<?= htmlspecialchars($s['keterangan']) ?>">
                                    </td>
    ...
    <script>
        ...
        formEl.addEventListener('submit', async function(ev) {
            ...
            try {
                ...
                if (shouldMap) {
                    const res2 = await fetch('/path/to/ajax_check_rename.php', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-Token': csrfToken
        ...
        function enableMassEdit() {
            document.querySelectorAll('#massEditForm input, #massEditForm select')
                .forEach(el => {
            ...
            document.getElementById('btnMassEdit').classList.add('hidden');
        ...
        async function saveMassEdit() {
            ...
            try {
                const res = await fetch('mass_edit_proses.php', {
                    method: 'POST',
                    body: formData
                });
                ...
                if (data.status === 'ok') {
                    alert('✅ Mass edit berhasil disimpan');
                    location.reload();
                } else {
